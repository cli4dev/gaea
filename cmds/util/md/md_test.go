...
func Test_getDBLink(t *testing.T) {
	type args struct {
		line *Line
	}
	tests := []struct {
		...
		// TODO: Add test cases.
		{name: "1", args: args{line: &Line{Text: "#### 2. 平台信息[plat_base_info](@dblink)", LineID: 1}}, want: "@dblink"},
		{name: "2", args: args{line: &Line{Text: "#### 2. 平台信息[plat_base_info]", LineID: 1}}, want: ""},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if got := getDBLink(tt.args.line); got != tt.want {
				t.Errorf("getDBLink() = %v, want %v", got, tt.want)
			}
		})
	}
}
